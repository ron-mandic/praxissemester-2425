...
function pyenv_install() {
    REQUIREMENTS_TXT="/requirements/mac/pyenv/requirements.txt"

    if [ -f "$REQUIREMENTS_TXT" ]; then
        pip3 install -r "$REQUIREMENTS_TXT"
    else
        DEFAULT_PACKAGES=("fastapi" "uvicorn[standard]" "torch" "torchvision" "torchaudio")
        
        for package in "${DEFAULT_PACKAGES[@]}"; do
            pip install "$package"
        done

        pip3 freeze > "$REQUIREMENTS_TXT"
    fi
}
