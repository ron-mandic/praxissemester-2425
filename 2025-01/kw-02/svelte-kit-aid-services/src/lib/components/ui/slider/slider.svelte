	...
	{#snippet children({ thumbs })}
		...
		{#each thumbs as thumb}
			<SliderPrimitive.Thumb
				...
				class={[
					{
						'block size-[18px] cursor-grab rounded-full border-2 border-primary bg-primary ring-offset-background transition-colors duration-200 ease-in focus-visible:bg-sidebar-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sidebar-ring focus-visible:ring-offset-0 active:bg-sidebar-primary active:outline-none active:ring-2 active:ring-sidebar-ring active:ring-offset-0 disabled:pointer-events-none disabled:opacity-50': true,
						'bg-sidebar-primary ring-2 ring-sidebar-ring': grab
					}
				]}
				onfocus={() => (grab = true)}
				onblur={() => (grab = false)}
